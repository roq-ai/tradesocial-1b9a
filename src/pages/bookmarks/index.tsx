...
import { getBookmarks, deleteBookmarkById } from 'apiSdk/bookmarks';
import { BookmarkInterface } from 'interfaces/bookmark';

type ColumnType = ColumnDef<BookmarkInterface, unknown>;
...
export function BookmarkListPage(props: BookmarkListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BookmarkInterface>>(
    () => `/bookmarks?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BookmarkInterface) => {
    if (hasAccess('bookmark', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/bookmarks/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/bookmarks/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('bookmark', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/bookmarks/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="bookmark" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Bookmarks
        </Text>
        {hasAccess('bookmark', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/bookmarks/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
