...
type ColumnType = ColumnDef<ChartInterface, unknown>;

interface ChartListPageProps {
...
export function ChartListPage(props: ChartListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ChartInterface>>(
    () => `/charts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ChartInterface) => {
    if (hasAccess('chart', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/charts/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'simulation', header: 'simulation', accessorKey: 'simulation' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/charts/view/${record.id}`} passHref legacyBehavior>
            <Button
