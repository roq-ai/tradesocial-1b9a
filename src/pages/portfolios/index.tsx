...
type ColumnType = ColumnDef<PortfolioInterface, unknown>;
...
export function PortfolioListPage(props: PortfolioListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PortfolioInterface>>(
    () => `/portfolios?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PortfolioInterface) => {
    if (hasAccess('portfolio', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/portfolios/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/portfolios/view/${record.id}`} passHref legacyBehavior>
            <Button
